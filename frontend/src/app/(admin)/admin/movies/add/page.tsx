...
export default function AdminAddMoviePage() {
  ...
  const onSave = () => {
    ...
    try {
      const existing = typeof window !== 'undefined' ? sessionStorage.getItem('movies') : null;
      const parsed: Array<{ id: number; title: string; date: string; time: string }> = existing
        ? JSON.parse(existing)
        : [];
  ...
  return (
    <div className="text-white" style={{ backgroundColor: '#1C1C1C', minHeight: '100vh' }}>
      ...
      {/* Form Card */}
      <div
        className="max-w-[53rem] mx-auto p-6 sm:p-8 rounded-md"
        style={{ backgroundColor: '#242424', border: '1px solid #FF478B' }}
      >
        {/* Movie Title + Type */}
