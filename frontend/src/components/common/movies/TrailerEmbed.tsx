...
export default function TrailerEmbed({ name, trailerUrl, isClosed, setIsClosed }: TrailerEmbedProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setIsClosed(true)} />
      
      {/* Popup Window */}
      <div className="flex flex-col items-center gap-y-8 w-4/5 h-4/5 relative bg-black/80 backdrop-blur-xl rounded-3xl border border-white/20 p-8 z-10 shadow-2xl">
        <button
          title="Close"
          type="button"
