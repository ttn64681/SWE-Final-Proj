...
export default function ACMLogo({ size = 'md', showHoverAnimation = true, className = '' }: ACMLogoProps) {
  ...
  const logoElement = (
    <span
      className={`font-pacifico leading-none ${sizeClasses[size]} ${className} ${
        size === 'xl' ? 'bg-gradient-to-r bg-clip-text text-transparent from-acm-pink to-acm-orange' : 'acm-gradient'
      }`}
      style={{
